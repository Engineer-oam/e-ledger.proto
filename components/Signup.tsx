...
import { UserRole, Sector, ERPType, State } from '../types';
import { REGISTRY_CONFIG, PHARMA_SUB_CATEGORIES, EXCISE_SUB_CATEGORIES } from '../constants';
...
const Signup: React.FC = () => {
  ...
  const handleFileUpload = () => {
    // Simulation
    setTimeout(() => setLicenseFile(formData.sector === Sector.EXCISE ? "excise_license_copy.pdf" : "drug_license_form_20.pdf"), 800);
  };
          ...
          <div className="relative z-10 space-y-8">
             <div className="space-y-4">
                {[
                    ...
                    { title: "Operational Role", desc: "Supply Chain Function" }, 
                    { title: "System Link", desc: "ERP Adapter" }
                ].map((s, idx) => (
                    <div key={idx} className={`flex items-center gap-4 group ${step - 2 === idx + 1 ? 'opacity-100' : 'opacity-40'}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all border ${step - 2 === idx + 1 ? 'bg-teal-500 border-teal-500 text-white shadow-[0_0_15px_rgba(20,184,166,0.5)]' : 'border-slate-600 text-slate-400'}`}>
                            {step - 2 > idx + 1 ? <Check size={14} /> : idx + 1}
                        </div>
                        <div>
                            <p className={`font-bold text-xs uppercase tracking-wider ${step - 2 === idx + 1 ? 'text-white' : 'text-slate-400'}`}>{s.title}</p>
          ...
          <form onSubmit={handleSubmit} className="h-full flex flex-col">
            ...
            {step === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">State Jurisdiction</label>
                    <select
                        value={formData.state}
                        onChange={(e) => setFormData({...formData, state: e.target.value})}
                        required
                        className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-teal-500 outline-none transition-all"
                    >
                ...
                <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-500 uppercase">{formData.sector === Sector.EXCISE ? 'Excise License Copy' : 'Business License / Registration'}</label>
                    <div 
                ...
                <div className="flex gap-3 pt-4 mt-auto">
                  <button type="button" onClick={() => navigate('/')} className="px-6 py-4 rounded-xl text-xs font-bold uppercase tracking-wider text-slate-500 hover:bg-slate-100 transition-colors">Cancel</button>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                ...
                <div className="space-y-2 pt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(formData.sector === Sector.EXCISE ? EXCISE_SUB_CATEGORIES : PHARMA_SUB_CATEGORIES).slice(0, 8).map(category => (
                        <button
                          key={category}
                          type="button"
